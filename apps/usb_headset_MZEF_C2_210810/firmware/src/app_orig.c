...
#ifdef __CORE_CM7_H_GENERIC
static __attribute__((aligned(32))) 
    DRV_I2S_DATA16 sinBuffer[APP_MAX_NO_SAMPLES_USB_FRAME] =
#else
static __attribute__((coherent)) __attribute__((aligned(16))) 
        ...
        switch (event)
        {
            ...
            case USB_DEVICE_AUDIO_EVENT_READ_COMPLETE:
            {
                       ...
                       APP_STATE_INITIAL_CODEC_WRITEREAD_REQUESTS)
                {
                    ...
                    {
                        SYS_PRINT("_USB: --READ COMPLETE QUEUE READY(R)\r\n");
            ...
            case USB_DEVICE_AUDIO_EVENT_WRITE_COMPLETE:
            {
                ...
                if (appData.state == 
                       APP_STATE_INITIAL_CODEC_WRITEREAD_REQUESTS)
                {
                    //if (appWRQueue.usbCompleteBufferLevel == 0x20)
                    //{
                    //    SYS_PRINT("USB: --Buffer Level is now 0x20 \r\n");
                    //}
                    if (_APP_USBReadAllBufferReady())
                    {
...
void APP_Tasks()
{
    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_INIT:
        {
            ...
            {
                SYS_MESSAGE("APP: ERROR - --Timer Handle Invalid");
                errorNum = ERR_TIMER;
                appData.state = APP_STATE_ERROR;
                break;
            }
        ...
        case APP_STATE_PROCESS_DATA:
        {
                ...
                APP_USB_MIC_RECORD_MONO))
            {
                ...
                if (appWRQueue.usbCompleteBufferLevel  == 
                    APP_QUEUE_SIZE)
                {
                    SYS_PRINT("APP: CODEC Playback Queue FULL(%d) h=0x%0x t=0x%0x\r\n", 
                                           APP_QUEUE_SIZE, headIdx, tailIdx);
                    queueFull = true;
                }
                     ...
                     (currentQueue->codecWRComplete == true) )
                {
                        ...
                        (audioErrWrite== USB_DEVICE_AUDIO_RESULT_OK))
                    {
                        ...
                        //++HEAD (Next USB RW)
                        if (appWRQueue.usbReadQueueCnt % 1000 == 0)
                        {
                            SYS_PRINT(
                    ...
                    (currentQueue->codecAecComplete == false))
                {
                    ...
                    //appData.codecClientWriteRead.handle = 
                        //DRV_CODEC_BUFFER_HANDLE_INVALID; 
#ifdef DEBUG_CODEC_LOOPBACK
                    //Loopback the Mic Record to the Headphone Playback
                    memcpy(currentPlayback, currentRecord,
                           appData.codecClientWriteRead.bufferSize);
#endif //DEBUG_CODEC_LOOPBACK
        ...
        case APP_STATE_MUTE_AUDIO_PLAYBACK:
        {
                   ...
                   APP_USB_MIC_RECORD_NONE))
            {
                ...
                appData.state = APP_STATE_IDLE;
                   ...
                   APP_USB_SPEAKER_PLAYBACK_STEREO) 
            {
                SYS_PRINT("APP: Mute Audio Off - INIT USB Reads\r\n");
                    ...
                    APP_USB_MIC_RECORD_NONE))
            {
                ...
                if (SYS_STATUS_READY == codecStatus)
                {
                        ...
                        appData.recordActive == false)
                    {
                        ...
                        {
                            SYS_MESSAGE("APP: --Codec Client Not Ready\r\n");
                        }
                    }    
                    appData.state = APP_STATE_IDLE;
                    break;
                        ...
                        APP_USB_SPEAKER_PLAYBACK_STEREO))
            {
                ...
                if (SYS_STATUS_READY == codecStatus)
                {
                    ...
                    if (DRV_CODEC_ClientReady(0) == true)
                    {
                        ...
                        SYS_MESSAGE("APP: --Codec Ready- Mute Off - HP Interface ON\r\n");
                        DRV_CODEC_MuteOff(appData.codecClientWriteRead.handle);
                        appData.muteStatus = false;
                        appData.state =  APP_STATE_INITIAL_USB_READ_REQUESTS;
                ...
                (appData.activeInterfaceAlternateSetting == 
                     APP_USB_SPEAKER_PLAYBACK_NONE))
            {
                SYS_MESSAGE("APP: -- HP Interface Off\r\n");
                appData.state = APP_STATE_IDLE;
            }
                        ...
                        APP_USB_MIC_RECORD_MONO))
            {
                ...
                if (SYS_STATUS_READY == codecStatus)
                {
                    ...
                    if (DRV_CODEC_ClientReady(0) == true)
                    {
                        ...
                        SYS_MESSAGE("APP: --Codec Ready - Mute Off - MP Interface ON-->\r\n");
                        _APP_Init_RWBufferQueue();
                        DRV_CODEC_MuteOff(appData.codecClientWriteRead.handle);
                        appData.muteStatus = false;
                ...
                (appData.activeMicInterfaceAlternateSetting == 
                     APP_USB_MIC_RECORD_NONE))
            {
                SYS_PRINT("APP: --MP Interface Off\r\n");
                appData.state = APP_STATE_IDLE;
            }
