...
#define APP_MAX_NO_OF_SAMPLES_IN_A_USB_FRAME USB_MAX_SAMPLE_RATE_HZ/USB_FRAME_SIZE_MS  //48  for 48Khz sampling  
...
#define APP_MAX_AEC_BUFFER_BYTES             APP_MAX_PLAYBACK_BUFFER_BYTES/2
...
typedef enum
{
    ...
    APP_STATE_INITIAL_CODEC_WRITEREAD_REQUEST,
...
//        For SAM must be aligned to 32 byte pages (use this) 
...
typedef struct {
    uint8_t __attribute__((aligned(32))) 
        playbackbuffer[APP_MAX_PLAYBACK_BUFFER_BYTES]; // bytes 
    uint8_t __attribute__((aligned(32))) 
        recordbuffer[APP_MAX_PLAYBACK_BUFFER_BYTES]; // bytes 
    int16_t __attribute__((aligned(32)))     
        aecbuffer[APP_MAX_AEC_BUFFER_BYTES]; // bytes 
    ...
    bool codecAecComplete; //1 byte bool usbReadComplete; //1 byte
    bool usbReadComplete; //1 byte
    bool usbWriteComplete; //1 byte
    char padding[14]; // bytes  (32 -18+14 = 0 )
} APP_RW_BUFFER;
...
typedef struct {
    ...
    uint32_t codecWriteQueueCnt;      //#of codec writeReads queued
    uint32_t codecWriteCompleteCnt;   //#of codec writeReads completed
    bool     usbInitialReadsCompleteFlag;
    uint8_t  padding[31];            // (8*4+1) % 32 = 1 
}APP_BUFFER_QUEUE;
