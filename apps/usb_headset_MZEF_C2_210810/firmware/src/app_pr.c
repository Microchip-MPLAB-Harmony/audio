...
#define DEBUG_TONE_CODEC  //Sin Tone Output
#define DEBUG_SAMPLECOUNT     14000
#undef DEBUG_AUDIO_STREAM

//#undef  DEBUG_USB_RW_LOOPBACK_TEST
//#undef  DEBUG_PLAYBACK_ONLY
//#undef  ALLOW_PLAYBACK_ONLY    
void DEBUG_PRINT1(char * strDst, int i);
void DEBUG_PRINT2(char * strDst, int i, char * strDst2, int i2);

char * paStr[] = { "pi", "PA" }; 
char * maStr[] = { "mi", "MA" }; 

static int printCnt;
static int printCnt2;
static int count = 0;
...
static int32_t usbPlaybackQueueLevel    = 0;
static int32_t usbRecordQueueLevel      = 0;
...
//==============================================================================
int _APP_SetUSBReadBufferReady(USB_DEVICE_AUDIO_TRANSFER_HANDLE handle);
int _APP_SetUSBWriteBufferReady(USB_DEVICE_AUDIO_TRANSFER_HANDLE handle);
//static void    _APP_SetAecBufferReady(int idx);
int _APP_SetCodecWriteReadComplete(DRV_CODEC_BUFFER_HANDLE handle);
void    _APP_Init_RWBufferQueue();
void    _APP_Init_QueueCodecStatus();
void    _APP_Cleari2sc0ReturnBuffer(DRV_CODEC_BUFFER_HANDLE handle);
int16_t _APP_GetNextIdx(int16_t index);
bool    _APP_USBReadAllBufferReady();
bool    _APP_USBInitAllBufferReady();

void _aecSumStereoChannels(DRV_I2S_DATA16 * input, 
                                  q15 * output, 
                                  int numSamples);
...
static volatile bool endrx_flag = false;
                               ...
                               uintptr_t context )
{
    ...
    switch( event )
    {
        ...
        {
            SYS_MESSAGE("APP: USB Resumed\r\n");
        }
        //break;  //Fall Through to ReAttach
    ...
    if ( iAudio == 0 )
    {
        ...
        switch (event)
        {
            ...
            case USB_DEVICE_AUDIO_EVENT_READ_COMPLETE:
            {
                ...
                //Now send this audio frame to Audio i2sc0 for Playback.
                readEventData = 
                ...
                } //End readEventData-status block
            }
            break;
                ...
                if(writeEventData->status == USB_DEVICE_AUDIO_RESULT_OK)
                {
                           ...
                           APP_STATE_INITIAL_CODEC_WRITEREAD_REQUESTS)
                    {
                        ...
                        if (_APP_USBInitAllBufferReady())
                        {
                            //SYS_MESSAGE("_USB: RW Queue READY\r\n");
                            SYS_MESSAGE("[UQRR]\r\n");
                            appWRQueue.usbInitialBufsReady = true; //ALL Ready for i2sc0 Playback
                        }
                    }
                }
            }
            ...
            case USB_DEVICE_AUDIO_EVENT_CONTROL_GET_CUR:
            {
                           ...
                           USB_SETUP_REQUEST_RECIPIENT_ENDPOINT)
                {
                    ...
                    if (controlSelector == USB_AUDIO_SAMPLING_FREQ_CONTROL)
                    {
                        ...
                        if (endpointNumber == MICROPHONE_EP)
                        {
                           //A control write transfer received from Host. 
                           //Now receive data from Host.
                           USB_DEVICE_ControlSend(appData.usbDevHandle, 
                        ...
                        else if (endpointNumber == HEADPHONE_EP)
                        {
                            //A control write transfer received from Host. Now receive data from Host.
...
void APP_Initialize()
{
    ...
    interruptStatus = __builtin_disable_interrupts(); 
        ...
        //appData.i2s.initialI2sc0WR          = 1;
        //appData.i2s.bufferSize              = APP_MAX_PLAYBACK_BUFFER_BYTES; 
        appData.usbReadBufSize              = APP_MAX_PLAYBACK_BUFFER_BYTES;
        appData.usbWriteBufSize             = APP_MAX_RECORD_BUFFER_BYTES;

        appData.codecClientWriteRead.handle  = DRV_HANDLE_INVALID; 
        ...
        appData.codecClientWriteRead.bufferSize = APP_MAX_PLAYBACK_BUFFER_BYTES;
    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_CODEC_SET_BUFFER_HANDLER:
        {
            ...
            interruptStatus = __builtin_disable_interrupts(); 
                ...
                //PDC_TxChannelCallbackRegister((PDC_CHANNEL_CALLBACK)NULL, 0);    
                //PDC_RxChannelCallbackRegister((PDC_CHANNEL_CALLBACK)TransferCompleteHandler, 0);    
...
DRV_CODEC_BufferEventHandlerSet(
            ...
            {
                SYS_PRINT("APP: --Codec I2S is Master\r\n");
            }
            else
            {
                SYS_PRINT("APP: --Codec I2S is Slave Fs = %d Hz\r\n",
                        appData.sampleFreq);
            }

                appData.codecConfigured = true;
                SYS_MESSAGE("Codec Configured\r\n");
        ...
        case APP_STATE_INITIAL_USB_READ_REQUESTS:
        {
                      ...
                      APP_USB_MIC_RECORD_MONO))
            {
                ...
                //SYS_MESSAGE("APP: USB Init RQ Req\r\n"); 

                //NOTE: Queue from the last head    
                ...
                interruptStatus = __builtin_disable_interrupts(); 
                    ...
                    for (i = 0; i < 8; i++)
                    {
                        ...
                        {
                            //Buffer Not Ready
                            char strDst[32] = "--RW FAIL i=";
                            DEBUG_PRINT1(strDst, i);
                            currentQueue->usbInUse = false;
                ...
                else
                {
                    ...
                    //errNum = ERR_USB;
                    //appData.state = APP_STATE_ERROR;
                    appData.state = APP_STATE_IDLE; //Start over
                    break;
                }
                    ...
                    APP_USB_MIC_RECORD_MONO))
            {
                if (appWRQueue.usbInitialBufsReady == true)
                {
                         ...
                         i++)
                    {
                        ...
                        } //End Buffer Ready Block

                    } //End Initial USB WR Queue Loop

                    if (errState == false)
                    {
                        SYS_PRINT(
                        "APP: Codec Initial WR Complete CWRQ=%d CWRC=%d URC=%d UWC=%d\r\n",
                                  appWRQueue.codecWRQueueCnt,
        ...
        case APP_STATE_PROCESS_DATA:
        {
                     ...
                     APP_USB_MIC_RECORD_MONO))
            {
                ...
                // HEAD --> Current Queue buffer to transfer after I2SC WR Completes
                headIdx = appWRQueue.headIdx;  //Queue HEAD
                //tailIdx  = appWRQueue.tailIdx;  //Queue TAIL
                currentQueue    = &appWRQueue.buffer[headIdx];
                currentPlayback = (DRV_I2S_DATA16 *) 
                     ...
                     (currentQueue->codecWRComplete == true) )
                {
...
#else
                        //I2SC0/Codec LOOPBACK The USB READ through the I2SC
                        ...
                        (audioErrWrite== USB_DEVICE_AUDIO_RESULT_OK))
                    {
                        ...
                        {
                            char strDst[32] = "usbWQ ";
                            DEBUG_PRINT1(strDst, appWRQueue.usbWriteQueueCnt+1);
                        }
                        if ((appWRQueue.usbReadQueueCnt+1) % 5000 == 0)
                        {
                            char strDst[32] = "usbRQ ";
                            DEBUG_PRINT1(strDst, appWRQueue.usbReadQueueCnt+1);
                        }
                    ...
                    (currentQueue->codecAecComplete == false))
                {
                    ...
                    if (skip >= 0)
                    {
                        SYS_PRINT("APP ----b=0x%04x (Cnt0x%04x)codecWRHandle = 0x%08x\r\n",
                                currentQueue->bufferIdx,
                                appWRQueue.codecWRQueueCnt, 
                                &currentQueue->codecWRHandle);
                        skip--;
                    }
                    ...
                    {
                        SYS_PRINT("APP: ERROR Codec WR Error\r\n");
                        errNum = ERR_CODEC_BUFFER;
                        appData.state = APP_STATE_ERROR;
                        break;
                    }
                    ...
                    {
                        char strDst[32] = "WRQC: ";
                        DEBUG_PRINT1(strDst, appWRQueue.codecWRQueueCnt);
                    }
        ...
        interruptStatus = __builtin_disable_interrupts(); 
                        ...
                        APP_USB_SPEAKER_PLAYBACK_STEREO))
            {
                ...
                //    (usbPlaybackQueueLevel > 0))
                {
                    usbPlaybackRestart = true;   //let WQ go to 0 restart
                }
                else
                {
                    usbPlaybackRestart = false;   //let WQ go to 0 restart
                }
                appData.playbackActive = true;
                ...
                if (SYS_STATUS_READY == codecStatus)
                {
                    ...
                    if (DRV_CODEC_ClientReady(0) == true)
                    {
                        ...
                        {
                            //Record Interface needs to clear for restart
                            usbRecordRestart = true;
                            SYS_MESSAGE("[PAMA]");
                        }
                ...
                {
                    //Code ERROR
                    SYS_MESSAGE("APP:  --Codec NOT Ready!\r\n");                   
                    errNum = ERR_CODEC;
                    appData.state = APP_STATE_ERROR;
                    break;
                }
        ...
        case APP_STATE_USB_QUEUE_CLEAR:
        {
            ...
            if (((usbPlaybackQueueLevel <= 0) || (usbPlaybackRestart == false)) &&
                ((usbRecordQueueLevel   <= 0) || (usbRecordRestart   == false)))
            {
                usbPlaybackQueueLevel = 0;
                usbRecordQueueLevel   = 0;
                usbPlaybackRestart = false;
    ...
    switch(event)
    {
        case DRV_CODEC_BUFFER_EVENT_COMPLETE:
        {
            ...
            if (bufferIdx != -1)
            {
                ...
                {
                    SYS_PRINT(
                        "    Codec Buffer %d has Index %d(0x%02x)- h=0x%08x\r\n",
                        appWRQueue.lastCWRCIdx, bufferIdx, bufferIdx, handle);
                }
            }
            else
            {
                SYS_PRINT(
                    "    Codec ERROR Buffer %d has Index %d(0x%02x) - h=0x%08x\r\n",
                    appWRQueue.lastCWRCIdx, bufferIdx, bufferIdx, handle);
            }
...
void _APP_Init_RWBufferQueue()
{
    ...
    for(i=0;i<APP_QUEUE_SIZE;i++)
    {
        ...
        appWRQueue.buffer[i].i2scTxComplete   = false;
        appWRQueue.buffer[i].i2scRxComplete   = false;
...
//Initialize i2sc0 Playback Buffer Queue
//******************************************************************************
void _APP_Init_QueueCodecStatus()
{
    int i=0;
    ...
    //appWRQueue.tailIdx     =  0;
    //appWRQueue.nextTailIdx =  1;
    //appWRQueue.lastCWRCIdx = -1;
    //appWRQueue.nextCWRCIdx =  0;

    for(i=0;i<APP_QUEUE_SIZE;i++)
    {
        if (appWRQueue.buffer[i].usbInUse == false)
        {
            appWRQueue.buffer[i].codecInUse       = false;
            appWRQueue.buffer[i].codecWRComplete  = false;
            appWRQueue.buffer[i].i2scTxComplete   = false;
    ...
    appData.i2s.bufferSize      = appData.usbReadBufSize;
    appData.i2s.initialI2sc0WR  = 1;
...
//******************************************************************************
//_APP_GetNextIdx()
//Increment the Head or Tail Index to i2sc0 Playback Buffer Queue
//******************************************************************************
int16_t _APP_GetNextIdx(int16_t index)
{
    int16_t newIndex; 
...
//******************************************************************************
void _APP_DupTheEmptyChannelFromMicrophone(DRV_I2S_DATA16 *input, 
...
//******************************************************************************
void _APP_DiscardTheEmptyChannelFromMicrophone2(DRV_I2S_DATA16 *input, 
                                                      DRV_I2S_DATA16 *output, 
                                                      int numSamples)
{
    int i;
...
void DEBUG_PRINT1(char * strDst, int i)
{
    ...
    SYS_CONSOLE_Message(SYS_CONSOLE_INDEX_0, strDst); 
}
...
void DEBUG_PRINT2(char * strDst, int i, char * strDst2, int i2)
{
    ...
    strcat(strDst,  strSrc);
    strcat(strDst2, strSrc2);
    strcat(strDst2, "\r\n");
    strcat(strDst,  strDst2);
    SYS_CONSOLE_Message(SYS_CONSOLE_INDEX_0, strDst); 
}
