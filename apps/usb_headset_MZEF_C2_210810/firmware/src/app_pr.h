...
#define MICROPHONE_EP      OUT_ENDPOINT    //0x81
#define HEADPHONE_EP       IN_ENDPOINT     //0x01 
...
typedef struct
{
    uint16_t initialI2sc0WR;
    uint16_t bufferSize;  //Bytes
} APP_I2SC0;
#endif //0
...
typedef struct {
    ...
    int16_t  nextTailIdx;                //Next codec WR buffer Idx 
    int16_t  lastCWRCIdx;             //Last Codec WR Complete Idx
    int16_t  nextCWRCIdx;             //Last Codec WR Complete Idx
...
#if 0
//void EndTxHandler(uintptr_t contextHandle);
void TransferCompleteHandler(uintptr_t contextHandle);
#endif //0
