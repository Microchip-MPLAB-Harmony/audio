...
loop:
	...
	lw	$t2, 0($a2)		# load indata2[i]

	lw	$t1, 4($a1)		# load indata1[i+1]
	lw	$t3, 4($a2)		# load indata2[i+1]

	mult	$t0, $t2		# outdata[i] = indata1[i] * indata2[i]

	lw	$t4, 8($a1)		# load indata1[i+2]
	lw	$t6, 8($a2)		# load indata2[i+2]

	mfhi	$t0

	mult	$t1, $t3		# outdata[i+1] = indata1[i+1] * indata2[i+1]

	lw	$t5, 12($a1)		# load indata1[i+3]
	lw	$t7, 12($a2)		# load indata2[i+3]

	mfhi	$t1

	mult	$t4, $t6		# outdata[i] = indata1[i+2] * indata2[i+2]
	...
	mult	$t5, $t7		# outdata[i+1] = indata1[i+3] * indata2[i+3]
	...
	sll	$t4, $t4, 1		# outdata[i+2] <<= 1
	sll	$t5, $t5, 1		# outdata[i+3] <<= 1

	sw	$t4,  8($a0)		# store outdata[i+2]
	sw	$t5, 12($a0)		# store outdata[i+3]
