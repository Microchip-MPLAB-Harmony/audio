...
loop:
	...
	lh	$t1, 2($a1)		# load indata[i+1]

	mul	$t0, $t0, $a2		# outdata[i] = indata[i] * c
	mul	$t1, $t1, $a2		# outdata[i+1] = indata[i+1] * c	

	lh	$t2, 4($a1)		# load indata[i+2]
	lh	$t3, 6($a1)		# load indata[i+3]

	addiu	$a1, $a1, 8		# 4 * sizeof(int16)

	mul	$t2, $t2, $a2		# outdata[i+2] = indata[i+2] * c
	mul	$t3, $t3, $a2		# outdata[i+3] = indata[i+3] * c
	...
	sra	$t2, $t2, 15		# outdata[i+2] >>= 15
	sra	$t3, $t3, 15		# outdata[i+3] >>= 15
