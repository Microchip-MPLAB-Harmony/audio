/*******************************************************************************
  ...
  Summary:
   Contains the functional implementation of mp3 advanced decoder.
...
#define MP3_DEC_C
#include "../mp3_advanced_lib_api.h"
#include "mp3_dec.h"

////////////////////////////////////////////////////////////////////////////////

void *mp3DecoderVars;
MP3_CHANNEL_INFO mp3Format;

MP3_STATE mp3State;
static MP3_DEC   mp3Decoder; // MP3 decoder structure
static uint16_t mp3ErrorCount;
static SYS_FS_HANDLE currentMP3FileHandle;
static DecoderEventHandlerCB decEventCB; // decoder event handler callback function
static int32_t findFirstFrameHeader(void *buf, uint32_t prevFilePtr);
...
{
    int mp3_heapsize = MP3DecoderGetStateSize();
    if ( size < mp3_heapsize )
    {
        return ( false );
    }
    else
    {
        ...
        unsigned int timerStart, timerEnd;
        uint32_t timetaken;
    ...
    *written = 0;
    ...
    switch ( mp3State )
    {
        case MP3_STATE_STREAM:
        {
            ...
            if ( MP3Decode ( mp3DecoderVars, input, inSize, read, &mp3Format, output, written ) == 0 ) //SUCCESS
            {
                ...
                *written *= 2;
                ...
                // for VBR MP3 files, each frame may have different bitrate 
                
#if defined(DISP_VBR)
                if ( mp3BitRate != mp3Format.bitRate )
                {
                    mp3BitRate = mp3Format.bitRate;
//                    if(!parse_frameheader)
                    {
                        // need bit rate to calculate track duration
                        MP3_EventHandler ( MP3_EVENT_BITRATE, mp3BitRate/1000 );
...
bool MP3_EventHandler ( MP3_EVENT event, uint32_t data )
{
    if(decEventCB == NULL)
    {
        // MP3 decoder need to be configured by APP Layer.
        return false;
    }
    switch ( event )
    {
...
static int32_t findFirstFrameHeader(void *buf, uint32_t filePtr){
    uint8_t *frame_cursor = buf;
    uint32_t ret = 1;
    uint32_t offset = 0;
    uint32_t in_block_offset = 0;
    ret = SYS_FS_FileRead(currentMP3FileHandle, frame_cursor, READBLOCKSIZE);
    while( memcmp ( frame_cursor, "ID3", 3 ) == 0)
    {
        // skip ID3 tag
        int32_t ID3TagSize =  ID3_Initialize(frame_cursor) + sizeof(ID3V2_TAG_HEADER);
        //offset+=ID3TagSize;
        filePtr+=ID3TagSize;
        SYS_FS_FileSeek(currentMP3FileHandle, filePtr, SYS_FS_SEEK_SET);
        
        ret = SYS_FS_FileRead(currentMP3FileHandle, frame_cursor, READBLOCKSIZE);
    }
    while(ret == READBLOCKSIZE){
        // exceed reading block
        if(in_block_offset == READBLOCKSIZE-1)
        {
            uint8_t tmp = frame_cursor[READBLOCKSIZE-1];
            in_block_offset = 0;
            frame_cursor[0] = tmp;
            ret = SYS_FS_FileRead(currentMP3FileHandle, frame_cursor+1, READBLOCKSIZE-1)+1;
        }
        ...
        //                              1 - Not protected
        // </editor-fold>
        if(frame_cursor[in_block_offset] == 0xFF && ((frame_cursor[in_block_offset+1]&0xE0) == 0xE0 || (frame_cursor[in_block_offset+1]&0xF0) == 0xF0))
        {
    ...
    SYS_FS_FileSeek(currentMP3FileHandle, filePtr+offset, SYS_FS_SEEK_SET);
